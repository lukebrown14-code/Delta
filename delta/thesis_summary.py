...
The *state* is always computed deterministically by :mod:`delta.thesis_health`;
...
from delta.theses import Thesis, accepted_items, get_thesis
from delta.thesis_health import HealthState, compute_health
...
async def summarize_thesis(delta: Any, thesis_id: str) -> ThesisSummary:
    ...
    draft, _result = await structured_mod.structured(
        delta.llm,
        task="thesis_summary",
        model=model_for(delta.cfg, "thesis_summary"),
