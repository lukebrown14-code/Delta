...
PLUGIN_GROUP = "delta.plugins"
TARGET_GROUP = "delta.targets"
LEGACY_WATCHLIST_GROUP = "delta.watchlists"
...
def discover_targets() -> dict[str, type[TargetPlugin]]:
    """Load target kinds from the ``delta.targets`` group, keyed by kind.

    The legacy ``delta.watchlists`` group is still honoured so third-party
