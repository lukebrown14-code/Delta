...
from delta.targets import DEFAULT_KIND, KNOWN_KINDS, LEGACY_KIND, WatchTarget, target_from_spec
from delta.theses import Thesis
...
def data_provider_status(delta: Any) -> list[DataProviderStatus]:
    ...
    for name, plugin in delta.plugins.items():
        ...
        table = dict(getattr(delta.cfg, "plugins", {}).get(name, {}))
        configured = all(
            not item.required
            or (
...
def configure_data_provider(
    delta: Any, name: str, values: dict[str, str], *, markets: list[str] | None = None
) -> None:
    ...
    reload_sources = getattr(delta, "reload_data_sources", None)
...
async def ingest(
    ...
    *,
    ...
    instruments = list(instruments) if instruments is not None else delta.universe()
    if market:
...
async def extract(
    ...
    *,
    ...
    """
    from delta.extract import extract_events
...
def brief_for(delta: Any, instrument_id: str) -> str | None:
    from delta.brief import build_brief

    instrument = next((item for item in delta.universe() if item.id == instrument_id), None)
    if instrument is None:
        return None
    brief = build_brief(delta.context(delta.universe()), instrument)
...
def data_health(delta: Any) -> DataHealth:
    tables = {
        "bar": BarTable,
...
def latest_report(reports_dir: str | Path, company: str) -> ReportStamp | None:
    ...
    """
    from delta.reports import read_report
...
def setup_checks(delta: Any) -> list[Check]:
    ...
    if spec is None:
        ...
        ok = bool(_provider_key(delta, spec)) and bool(getattr(delta.cfg, "llm_base_url", ""))
        checks = [
            Check(
                f"LLM provider ({provider})",
                ok,
    ...
    checks.append(
        Check(
            "SEC EDGAR contact",
            bool(delta.cfg.plugins.get("sec_edgar", {}).get("contact")),
            "Set [plugins.sec_edgar].contact",
        )
    )
    health = data_health(delta)
...
def thesis_fleet(engine: Any, *, now: datetime | None = None) -> list[ThesisHealth]:
    ...
    from delta.evidence import evidence_by_ids
    from delta.thesis_health import compute_health
