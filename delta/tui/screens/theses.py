...
class Theses(DeltaScreen):
    ...
    def compose_content(self) -> ComposeResult:
        with PaneRow(id="thesis-split"):
            with Pane(title="theses", id="thesis-claims"):
                ...
                yield DeltaTable(id="thesis-table")
            ...
            with Pane(title="evidence", key="e", id="thesis-evidence-pane"):
                ...
                yield DeltaTable(id="thesis-ledger")
    ...
    def _sync_columns(self) -> bool:
        ...
        table = self.query_one("#thesis-table", DeltaTable)
        ...
        ledger = self.query_one("#thesis-ledger", DeltaTable)
    ...
    def _style(self, token: str) -> str:
        ...
        Under the Delta theme every ``text-*`` token is a hex colour. Under a
    ...
    def reload_theses(self) -> None:
        ...
        """
        self._theses = theses.list_theses(self.delta.engine)
        now = datetime.now(UTC)
        self._health = {}
        self._pending = {}
        for thesis in self._theses:
            links = theses.evidence_for(self.delta.engine, thesis.id, accepted_only=False)
            ...
            by_id = {
                ...
                for item in evidence_by_ids(
                    self.delta.engine, [link.evidence_id for link in accepted_links]
                )
            }
            accepted = [
        ...
        try:
            if editing:
                if self.selected is None:
                    return
                thesis = theses.update_thesis(self.delta.engine, self.selected, **fields)
            else:
                claim = fields.pop("claim")
                thesis = theses.create_thesis(self.delta.engine, claim, **fields)
    ...
    async def _summarise(self) -> None:
        ...
        try:
            summary = await thesis_summary.summarize_thesis(self.delta, selected)
    ...
    def _current_row(self) -> theses.ThesisEvidence | None:
        table = self.query_one("#thesis-ledger", DeltaTable)
    ...
    async def _resolve(self, action: str) -> None:
        ...
        if action == "unaccept":
            if not row.accepted:
                ...
                return
            theses.set_accepted(self.delta.engine, row.thesis_id, row.evidence_id, False)
        else:
            if row.accepted:
                self.notify("already accepted — press u to return it to pending")
                return
            if action == "accept":
                theses.set_accepted(self.delta.engine, row.thesis_id, row.evidence_id, True)
            else:
                theses.remove_evidence(self.delta.engine, row.thesis_id, row.evidence_id)
        self._summary = None
    ...
    async def _render_detail(self) -> None:
        ...
        if self.selected is None:
            ...
            self._paint_ledger_badge()
            return
        thesis = theses.get_thesis(self.delta.engine, self.selected)
        self._links = theses.evidence_for(self.delta.engine, thesis.id, accepted_only=False)
        links = self._links
        self.candidates = [r for r in links if not r.accepted]
        items = evidence_by_ids(self.delta.engine, [r.evidence_id for r in links])
