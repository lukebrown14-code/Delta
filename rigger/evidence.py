...
# Source provenance stays on the stored row.  This small registry makes
# primary disclosures visible to readers without a database migration.
PRIMARY_DISCLOSURE_SOURCES = frozenset({"sec_edgar", "asx_announcements"})
FILING_SOURCE = "sec_edgar"  # compatibility alias
...
def _news(
    ...
    if kind == "news":
        stmt = stmt.where(~col(NewsItemTable.source).in_(PRIMARY_DISCLOSURE_SOURCES))
    elif kind == "filing":
        stmt = stmt.where(col(NewsItemTable.source).in_(PRIMARY_DISCLOSURE_SOURCES))
...
def _news_item(row: NewsItemTable) -> EvidenceItem:
    ...
    kind: EvidenceKind = "filing" if row.source in PRIMARY_DISCLOSURE_SOURCES else "news"
    return EvidenceItem(
        id=f"{kind}:{row.id}",
