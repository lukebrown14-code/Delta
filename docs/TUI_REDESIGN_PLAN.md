# Delta TUI Redesign — "Terminal Desk"
...
> cause in *Context* below is already fixed: `delta/tui/theme.py`,
> `delta/tui/shell.py` and a 337-line `delta/tui/widgets.py` all exist,
> `delta.tcss` is 153 lines of token-based rules, `DeltaCommands` is wired
> into the command palette, `DeltaScreen.on_screen_resume` re-runs
...
1. **`delta/tui/app.py` never mounts `Header()` or `Footer()`.** All 11 `BINDINGS` (`app.py:28-40`) are invisible. The only way to learn the app is to press `?`, and `screens/help.py:9` is a hand-maintained duplicate of the binding list that can silently drift.
...
3. **`delta/tui/delta.tcss` is 39 lines of padding and margin.** No palette, no borders, no theme, no dark-mode handling. Colour appears only as inline Rich markup inside strings (`[bold cyan]DELTA[/bold cyan]`, `home.py:21`).
...
|---|---|
| `DeltaApp(delta)` constructor signature | `test_tui.py:58` |
...
Also preserve the **standalone-screen property** documented in `theses.py:1-6` and `reports.py:18-22`: those screens must still mount under any App without `app.py`'s stylesheet. Keep their `DEFAULT_CSS`/`CSS`, but rewrite the rules to use `$`-tokens so they inherit the theme when run inside `DeltaApp`.
...
### `delta/tui/theme.py` (new)

Register a `textual.theme.Theme` named `delta-dark` and set `self.theme` in `DeltaApp.on_mount`. Ayu-Dark-derived, chosen for contrast at small sizes:
...
Add a matching `delta-light` so `App.theme` can toggle; the token names stay identical so no CSS changes.

### `delta/tui/delta.tcss` (rewrite)
...
### `delta/tui/widgets.py` (replace the dead `Panel`)
...
- **`Pill(Static)`** — small inline token for kinds, statuses and thesis health. Reuse the existing `state_style()` / `badge_text()` from `delta/thesis_health.py` rather than inventing new styling — `theses.py:233-251` already does this correctly.
...
### `delta/tui/shell.py` (new)

- **`TopBar(Horizontal)`** — brand `DELTA`, then right-aligned status cells: data freshness (`StatusDot` + age, from `services.data_health`), active model id, session spend (from `services.llm_costs`). Refreshed on a `set_interval`.
...
- **`DeltaScreen(Screen)`** — base class whose `compose()` yields `TopBar`, `Horizontal(NavRail, <content>)`, `Footer()`, and delegates the middle to an abstract `compose_content()`.
...
- Add a `DeltaCommands(Provider)` so the built-in command palette (`ctrl+p`) can jump to any screen and run Gather/Report — discoverability without more chrome.
- **Fix the stale-data bug:** `refresh_view()` is currently only called from `on_mount`, and all eight screens are constructed eagerly at startup (`app.py:51-63`), so every screen shows data frozen at launch. Add an `on_screen_resume` hook in `DeltaScreen` that calls `refresh_view()` when defined.
...
2. **Lint** — `uv run ruff check delta/tui`.
3. **Live run** — `uv run delta`, then walk the flow from `PROJECT_SPEC`: add a target on Targets → check Data → generate on Reports → ask on Chat. Confirm the rail highlights the active screen and the Footer shows keys on every screen.
4. **Dev tooling** — `uv run textual run --dev delta.tui.app:DeltaApp` with `uv run textual console` in a second pane for live CSS edits and layout warnings.
...
1. `feat(tui): theme + design tokens` — `theme.py`, `delta.tcss`, no layout change
