...
- Produce a new `Signal` (new id, `ts` now) copying the base fields, with `strategy = f"critic:{wraps}"`, `conviction = revised_conviction`, `direction = "flat"` when `verdict == "reject"`, and `metadata["critic"] = {base_signal_id, original_conviction, counter_argument, risks, verdict}` (the critic model and `prompt_version = "critic_v1"` live on the signal's top-level `model`/`prompt_version` fields)``. `cost_usd` = base cost + critique cost. Both base and critic signals are returned so both are stored and can be compared in Phase 3.
